...
(() => {
  ...
  const effects={
    ...
    brat:{family:families.brat,weight:700,min:36,max:150,lineHeight:.98,tracking:-.035,align:'center',case:'none'},
    eternal:{family:families.eternal,weight:400,min:34,max:150,lineHeight:1,tracking:0,align:'left',case:'none'},
    aurora:{family:families.aurora,weight:400,min:38,max:150,lineHeight:1,tracking:-.01,align:'center',case:'center'},
    ...
    fadeup:{family:families.fadeup,weight:400,min:34,max:150,lineHeight:1.10,tracking:-.012,align:'center',case:'none'},
  ...
  const ready=(async()=>{if(!document.fonts?.ready)return true;await document.fonts.ready;const results=await Promise.all(fontFaces.map(face=>document.fonts.load(face).then(()=>true).catch(()=>false)));if(results.some(ok=>!ok))console.warn('KEFE: one or more bundled fonts failed to load',fontFaces.filter((_,i)=>!results[i]));return results.every(Boolean);})();
