...
(()=>{
  ...
  const state={rendering:false,installing:false,snapshotRestored:false,lastEffect:null,lastMs:0};
  ...
  function handoff(ms){
    ...
    if(!lyrics)return;
    let active=false;
    try{active=ms>=entranceMs()}catch{}
    if(active){
      ...
      lyrics.classList.add('visible');lyrics.classList.remove('preenter');
      lyrics.style.setProperty('visibility','visible');
      lyrics.style.setProperty('z-index','4');
    }else if(title){
      title.hidden=false;title.removeAttribute('aria-hidden');title.style.removeProperty('display');
      lyrics.style.removeProperty('visibility');lyrics.style.removeProperty('z-index');
    }
  }
  ...
  function enforceAppleContext(ms){
    ...
    let active=0;
    try{active=clamp(ci(ms),0,lines.length-1)}catch{}
    const lo=Math.max(0,active-cw.before),hi=Math.min(lines.length-1,active+cw.after);
    document.querySelectorAll('#lyrics .apple-line').forEach((el,domIndex)=>{
      const parsed=Number(el.dataset.line);
      const lineIndex=Number.isFinite(parsed)?parsed:domIndex;
      el.style.visibility=lineIndex>=lo&&lineIndex<=hi?'visible':'hidden';
    });
    const meta=$('#activeMeta');
    if(meta)meta.textContent=`Lyric ${active+1} of ${lines.length} · ${cw.total===1?'current only':`${cw.total} on screen`}`;
  }
  ...
  function fallback(ms,error,key){
    ...
    if(key==='charli'){
      lyrics.innerHTML=`<div class="charli-frame"><div class="charli-card charli-dark">${esc(lines[i]?.text||'')}</div></div>`;
      return;
    }
  ...
  function render(ms=0){
    ms=Math.max(0,Number(ms)||0);state.lastMs=ms;
    if(state.rendering)return;
    state.rendering=true;
    const key=getEffect();
    try{
      syncEffectUI(key);
      cleanupEnhancers(key);
      const base=key==='charli'?effectRender:appleRender;
      if(typeof base!=='function')throw new Error('No base lyric renderer is available');
      base(ms);
      applyTransformState();
      handoff(ms);
      ensureBackground();
      if(key==='apple'){
        enforceAppleContext(ms);
        window.linaAppleLetterHighlight?.apply?.(ms);
      }else if(key==='eternal'){
        window.linaEternalSunshine?.apply?.(ms);
        const meta=$('#activeMeta');
        if(meta&&Array.isArray(lines)&&lines.length){let i=0;try{i=clamp(ci(ms),0,lines.length-1)}catch{}const count=selectedContext().total;meta.textContent=`Eternal Sunshine · Lyric ${i+1} of ${lines.length} · ${count===1?'current only':`${count} on screen`}`}
      }
      state.lastEffect=key;
      return true;
    }catch(error){
      fallback(ms,error,key);
      handoff(ms);ensureBackground();
      try{if(key==='apple')window.linaAppleLetterHighlight?.apply?.(ms);else if(key==='eternal')window.linaEternalSunshine?.apply?.(ms)}catch{}
      return false;
    }finally{state.rendering=false}
  }

  function setEffect(value,{redraw=true,dirty=true}={}){
    if(!VALID_EFFECTS.has(value))value='apple';
    syncEffectUI(value);
  ...
  function captureLayout(){
    ...
    return{
      ...
      align:$('#textAlign')?.value,lineHeight:$('#lineHeight')?.value,spacing:$('#letterSpacing')?.value,
  ...
  function applyLayoutValues(values,{dirty=false,redraw=true}={}){
    const key=VALID_EFFECTS.has(values.key)?values.key:getEffect();
    const studio=window.linaConsolidatedState;
    if(studio){
      studio.x=Number(values.x)||0;studio.y=Number(values.yDrag)||0;studio.scale=Number(values.scale)||1;studio.rot=Number(values.rot)||0;
    }
    setControl('studioScale',Math.round((Number(values.scale)||1)*100));
    setControl('studioRotation',Number(values.rot)||0);
    setControl('size',values.size??safeSize(key));
    setControl('yPos',values.y??'50');
    setControl('contextMode',values.view??DEFAULTS[key].view);
    setControl('textAlign',values.align??DEFAULTS[key].align);
    setControl('lineHeight',values.lineHeight??DEFAULTS[key].lineHeight);
    setControl('letterSpacing',values.spacing??DEFAULTS[key].spacing);

    const typography=window.linaTypographyState?.[key];
    if(typography){
      typography.align=String(values.align??DEFAULTS[key].align);
      typography.lineHeight=String(values.lineHeight??DEFAULTS[key].lineHeight);
      typography.spacing=String(values.spacing??DEFAULTS[key].spacing);
    }

    const lyrics=$('#lyrics');
    if(lyrics){
      lyrics.style.top=`${values.y??50}%`;
      lyrics.style.textAlign=String(values.align??DEFAULTS[key].align);
      lyrics.style.lineHeight=String(values.lineHeight??DEFAULTS[key].lineHeight);
      lyrics.style.letterSpacing=`${values.spacing??DEFAULTS[key].spacing}em`;
    }
    applyTransformState();
    try{window.linaSyncTypography?.()}catch{}
    try{window.linaQuickSettingsSync?.()}catch{}
  ...
  function ownEffectPicker(){
    ...
    if(!picker||picker.dataset.linaOwner==='canonical')return false;
    picker.dataset.linaOwner='canonical';
    picker.addEventListener('click',e=>{
      const button=e.target.closest?.('.effect-option[data-effect]');if(!button)return;
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();setEffect(button.dataset.effect);
    },true);
    return true;
  }
  ...
  const observer=new MutationObserver(()=>installControlOwnership());
  ...
  window.linaRuntime={
    version:'canonical-v1',render,setEffect,getEffect,resetLayout,selectedContext,contextForValue,
    ensureBackground,applyLayoutValues,captureLayout,state,
    selfTest(){
      const modes=['current','3','5','7','9'].map(contextForValue);
      return{
        renderOwner:document.documentElement.dataset.renderOwner,
        effect:getEffect(),
        appleBase:typeof appleRender==='function',charliBase:typeof effectRender==='function',
        appleEnhancer:!!window.linaAppleLetterHighlight,eternalEnhancer:!!window.linaEternalSunshine,
        contextModes:modes.map(x=>x.total),
        resetOwner:$('#quickResetLayout')?.dataset.linaOwner||'pending'
      };
    }
  };
  window.linaResetLyricLayout=resetLayout;
  document.documentElement.dataset.renderOwner='canonical-v1';
  document.documentElement.dataset.effectOwner='canonical-v1';
  document.documentElement.dataset.layoutOwner='canonical-v1';
  syncEffectUI(getEffect());
  requestAnimationFrame(()=>render((Number($('#audio')?.currentTime)||0)*1000));
})();
