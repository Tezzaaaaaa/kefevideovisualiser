...
(()=>{
  ...
  const requiredIds=['nav','play','stop','audioFile','lyricsText','story','lyrics','exportBtn','exportBottomBtn'];
  ...
  if(!window.linaPreviewRecovery)handlerFailures.push('preview-recovery');
  if(typeof window.linaExportState!=='function')handlerFailures.push('export-lifecycle');
  if(!window.__linaStudio)handlerFailures.push('signature-effects');
  if(!window.linaControlAudit)handlerFailures.push('control-audit');
  else if(window.linaControlAudit.missing?.length)handlerFailures.push(...window.linaControlAudit.missing.map(x=>`control:${x}`));
  if(missing.length||missingFns.length||handlerFailures.length){
    console.error('LINA startup check failed',{missing,missingFns,handlerFailures,controlAudit:window.linaControlAudit});setStatus(`LINA startup QA failed${handlerFailures.length?` · ${handlerFailures.length} issue${handlerFailures.length===1?'':'s'}`:''}.`);document.documentElement.dataset.linaReady='false';return;
  }
  ...
  console.info('LINA startup check passed',{panels:navButtons.length,audioReady:!!audio,exportReady:true,exportLifecycle:true,mediaLifecycle:true,previewRuntime:true,previewRecovery:true,signatureEffects:true,controls:window.linaControlAudit});
})();
