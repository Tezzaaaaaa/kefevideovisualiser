...
(()=>{
  ...
  function showPanel(step){
    ...
    document.querySelectorAll('.tool[data-panel]').forEach(panel=>{
    ...
    document.querySelectorAll('#nav .navbtn[data-tool]').forEach((button,i)=>{
      const on=button.dataset.tool===id;
      button.classList.toggle('active',on);
      button.setAttribute('aria-selected',on?'true':'false');
      button.tabIndex=on?0:-1;
      if(i<index)button.classList.add('done');
    });
    ...
    if(next)next.textContent=index===STEPS.length-1?'Review':'Next';
    document.querySelector('.left')?.scrollTo({top:0,behavior:'auto'});
  }
