...
(()=>{
  ...
  function showPanel(step){
    ...
    const prev=document.querySelector('#prevStep'),next=document.querySelector('#nextStep');if(prev)prev.disabled=index===0;if(next){next.textContent=index===STEPS.length-1?'Preview':'Next';next.dataset.linaBound='panel-nav'}
    document.querySelector('.left')?.scrollTo({top:0,behavior:'auto'});updateDock(index);
  }
  function showPreview(){
    const stage=document.querySelector('.stage');
    if(stage){stage.scrollIntoView({behavior:'smooth',block:'start'});stage.setAttribute('tabindex','-1');setTimeout(()=>stage.focus({preventScroll:true}),250)}
  }
  ...
  document.addEventListener('click',e=>{
    ...
    if(e.target.closest('#nextStep')){e.preventDefault();e.stopImmediatePropagation();const i=Math.max(0,STEPS.indexOf(document.querySelector('#nav .navbtn.active')?.dataset.tool||'setup'));if(i<STEPS.length-1)showPanel(STEPS[i+1]);else showPreview();return}
  },true);
  document.querySelectorAll('#nav .navbtn[data-tool],#prevStep').forEach(el=>el.dataset.linaBound='panel-nav');
