...
// Apple Music motion is driven only by playback time: line flow, word focus and glyph illumination.
...
function drawCover(ctx,media,w,h,blur){
  const mw=media.videoWidth||media.width,mh=media.videoHeight||media.height;if(!mw||!mh)return;
...
function drawTimedWord(ctx,ref,x,y,style,time){
  const text=ref.text,textWidth=ref.textWidth,start=Number(ref.time),end=Number(ref.endTime),duration=Math.max(.06,end-start),completed=time>=end,active=time>=start&&time<end,focus=wordFocus(ref,time),scale=1+focus*.045;
...
function drawAppleBlock(ctx,w,yCenter,style,block,alpha,time){
  ...
  for(const row of block.rows){
    ...
    for(const ref of row){
      ...
      else drawTimedWord(ctx,ref,x,y,style,time);
      x+=ref.advance;
    }
    y+=block.lineHeight;
  }
...
function drawBrat(ctx,w,h,style,sync){if(!sync.line)return;ctx.save();ctx.font=`400 ${style.fontSize}px "Arial Narrow",Arial,sans-serif`;ctx.textBaseline="middle";ctx.textAlign=style.align;ctx.fillStyle="#101010";ctx.filter="blur(.4px)";const x=textX(style.align,w,w*.08),rows=wrapText(ctx,String(sync.line.text||"").toLowerCase(),w*.72);ctx.translate(x,h*.5);ctx.scale(1.1,.9);rows.forEach((row,i)=>ctx.fillText(row,0,(i-(rows.length-1)/2)*style.fontSize*1.05));ctx.restore()}
function drawEternal(ctx,w,h,style,sync){if(!sync.line)return;ctx.save();ctx.font=`${style.fontSize}px cursive`;ctx.textBaseline="middle";ctx.textAlign=style.align;ctx.fillStyle=style.textColor;const x=textX(style.align,w,w*.1),rows=wrapText(ctx,applyCase(sync.line.text,style.textCase),w*.8);rows.forEach((row,i)=>ctx.fillText(row,x,h*.5+(i-(rows.length-1)/2)*style.fontSize*style.lineSpacing));ctx.restore()}

export function render(ctx,w,h,appState,mediaCache){
  ctx.clearRect(0,0,w,h);drawBackground(ctx,w,h,appState.background,mediaCache);const time=appState.playback.currentTime,sync=getSyncState(appState.lyrics.lines,time),base=appState.style,profile=base.effects?.[base.effect]||{},style={...base,...profile};if(style.letterSpacing)ctx.letterSpacing=`${style.letterSpacing}px`;
  switch(base.effect){case"apple":drawApple(ctx,w,h,style,sync,appState.lyrics.lines,time);break;case"brat":drawBrat(ctx,w,h,style,sync);break;case"eternal":drawEternal(ctx,w,h,style,sync);break}
