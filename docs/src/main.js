...
const fmt = time => `${Math.floor((time || 0) / 60)}:${String(Math.floor((time || 0) % 60)).padStart(2, "0")}`;
...
function readiness() {
  const ready = Boolean(state.audio.file && state.lyrics.lines.length > 0 && (media.image || media.video));
  $("#readyStatus").textContent = ready ? "Ready to export" : "Add audio, lyrics and background";
...
function songFromFilename(name) {
  ...
  return parts.length > 1
    ? { artist: parts[0].trim(), track: parts.slice(1).join(" - ").trim() }
    : { artist: "", track: base };
}

function activeEffectStyle() {
  const effect = state.style.effect;
  if (!state.style.effects[effect]) state.style.effects[effect] = { fontSize: 76, align: "center" };
  return state.style.effects[effect];
}

function syncEffectControls() {
  const profile = activeEffectStyle();
  $("#lyricSize").value = profile.fontSize;
  $("#lyricSizeValue").textContent = String(profile.fontSize);
  document.querySelectorAll("[data-align]").forEach(button => {
    button.classList.toggle("active", button.dataset.align === profile.align);
  });
}

function syncHazeControls() {
  $("#hazeEnabled").checked = Boolean(state.background.hazeEnabled);
  $("#hazeColor").value = state.background.hazeColor || HAZE_WHITE;
  const pct = Math.round(clamp(state.background.hazeOpacity || 0, 0, 1) * 100);
  $("#hazeOpacity").value = String(pct);
  $("#hazeOpacityValue").textContent = `${pct}%`;
  document.querySelectorAll("[data-haze-preset]").forEach(button => {
    ...
    button.classList.toggle(
      "active",
      Boolean(state.background.hazeEnabled) &&
        String(state.background.hazeColor).toUpperCase() === target,
    );
  });
}
...
async function ensureActiveEffectFont() {
  ...
  await Promise.race([
    ...
    new Promise((_, reject) => setTimeout(
      () => reject(new Error("Homemade Apple could not be loaded. Check your connection and try again.")),
      5000,
    )),
  ]);
  if (!document.fonts.check(`${size}px "Homemade Apple"`)) {
    throw new Error("Homemade Apple could not be loaded. Check your connection and try again.");
  }
}

function wrappedVideoTime(time, duration) {
  if (!Number.isFinite(duration) || duration <= 0) return 0;
  return ((Number(time) || 0) % duration + duration) % duration;
}

function circularDrift(current, target, duration) {
...
function syncBackgroundClock(targetTime, shouldRun) {
  ...
  if (shouldRun) {
    if (distance > 0.10 && !video.seeking) video.currentTime = target;
    video.playbackRate = distance <= 0.10 ? clamp(1 + drift * 0.30, 0.97, 1.03) : 1;
    if (video.paused) video.play().catch(() => {});
  } else {
    video.playbackRate = 1;
    if (!video.paused) video.pause();
    if (distance > 0.018 && !video.seeking) video.currentTime = target;
  }
}
...
async function findLyrics() {
  ...
  $("#lyricsStatus").textContent = "Finding synced lyrics…";
  $("#findLyrics").disabled = true;
  ...
  try {
    ...
    state.lyrics.raw = match.syncedLyrics;
    state.lyrics.format = parsed.format;
    state.lyrics.lines = parsed.lines;
    state.project.title = match.trackName || track;
    state.project.artist = match.artistName || artist;

    const firstLyricTime = parsed.lines.find(line => Number.isFinite(line.time))?.time;
    if (audio.paused && Number.isFinite(firstLyricTime)) {
      audio.currentTime = Math.min(audio.duration || firstLyricTime, firstLyricTime + 0.05);
    }

    const hasWordTiming = parsed.lines.some(line => line.words?.length);
    $("#lyricsStatus").textContent = hasWordTiming
      ? `${parsed.lines.length} word-synced lines found`
      : `${parsed.lines.length} synced lines found`;
    toast("Synced lyrics loaded");
    readiness();
  } catch (error) {
    $("#lyricsStatus").textContent = "Could not find synced lyrics automatically";
    toast(error.message, true);
  } finally {
    $("#findLyrics").disabled = false;
  }
}
...
$("#audioInput").addEventListener("change", event => {
  ...
  $("#lyricsStatus").textContent = "Waiting for audio";
  readiness();
});
...
audio.addEventListener("loadedmetadata", () => {
  ...
  $("#lyricsStatus").textContent = "Ready to find lyrics";
  $("#findLyrics").disabled = false;
  void findLyrics();
});
...
$("#backgroundInput").addEventListener("change", event => {
  ...
  media.video = null;
  readiness();
  $("#backgroundStatus").textContent = `Loading ${file.name}…`;

  if (file.type.startsWith("video/")) {
    backgroundVideo.pause();
    backgroundVideo.playbackRate = 1;
    backgroundVideo.onloadedmetadata = () => {
      if (loadId === backgroundLoadId && backgroundVideo.duration > 0) {
        backgroundVideo.currentTime = wrappedVideoTime(audio.currentTime, backgroundVideo.duration);
      }
    };
    backgroundVideo.onloadeddata = () => {
      if (loadId !== backgroundLoadId) return;
      media.video = backgroundVideo;
      state.background.type = "video";
      $("#backgroundStatus").textContent = file.name;
      syncBackgroundClock(audio.currentTime, !audio.paused);
      readiness();
    };
    backgroundVideo.onerror = () => {
      if (loadId !== backgroundLoadId) return;
      $("#backgroundStatus").textContent = "Video could not be loaded";
      toast("This background video format is not supported on this device", true);
      readiness();
    };
    ...
    image.onerror = () => {
      if (loadId !== backgroundLoadId) return;
      $("#backgroundStatus").textContent = "Image could not be loaded";
      toast("This background image could not be loaded", true);
      readiness();
    };
...
$("#effects").addEventListener("click", event => {
  ...
  if (!button) return;
  state.style.effect = button.dataset.effect;
  if (state.style.effect === "eternal") void ensureActiveEffectFont().catch(() => {});
  document.querySelectorAll("[data-effect]").forEach(item => item.classList.toggle("active", item === button));
  syncEffectControls();
  syncHazeControls();
});
...
$("#alignment").addEventListener("click", event => {
  ...
  if (!button) return;
  activeEffectStyle().align = button.dataset.align;
  syncEffectControls();
});

$("#lyricSize").addEventListener("input", event => {
  const value = Number(event.target.value);
  activeEffectStyle().fontSize = value;
  $("#lyricSizeValue").textContent = String(value);
  if (state.style.effect === "eternal") void ensureActiveEffectFont().catch(() => {});
});

$("#hazeEnabled").addEventListener("change", event => {
  state.background.hazeEnabled = event.target.checked;
  syncHazeControls();
});

$("#hazeColor").addEventListener("input", event => {
  state.background.hazeColor = event.target.value.toUpperCase();
  state.background.hazeEnabled = true;
  syncHazeControls();
});

$("#hazeOpacity").addEventListener("input", event => {
  state.background.hazeOpacity = Number(event.target.value) / 100;
  syncHazeControls();
});

$("#hazePresets").addEventListener("click", event => {
  const button = event.target.closest("[data-haze-preset]");
  if (!button) return;
  state.background.hazeColor = button.dataset.hazePreset === "white" ? HAZE_WHITE : BRAT_GREEN;
  state.background.hazeEnabled = true;
  syncHazeControls();
});

$("#aspects").addEventListener("click", event => {
  const button = event.target.closest("[data-aspect]");
  if (!button) return;
  state.canvas.aspect = button.dataset.aspect;
  const size = ASPECTS[state.canvas.aspect];
  canvas.width = size.w;
...
$("#seek").addEventListener("input", event => {
  ...
  syncBackgroundClock(audio.currentTime, false);
});

async function startExport() {
  if (exportJob || $("#exportBtn").disabled) return;
  try {
    await ensureActiveEffectFont();
  } catch (error) {
    toast(error.message, true);
    return;
  }
  ...
  try {
    exportJob = await exportVideo({
      ...
      onProgress: progress => {
        $("#exportPct").textContent = `${Math.round(progress * 100)}%`;
      ...
      onDone: (blob, format) => {
        ...
        downloadBlob(
          blob,
          `${(state.project.title || "LINA lyric video").replace(/[^a-z0-9 _-]/gi, "")}.${format.extension}`,
        );
...
$("#cancelExport").addEventListener("click", () => exportJob?.cancel());
...
state.canvas.aspect = "9:16";
state.style.effect = "apple";
state.style.textColor = "#FFFFFF";
state.style.accentColor = "#FFFFFF";
state.style.dimColor = "rgba(255,255,255,.32)";
state.background.dim = 0.35;
syncEffectControls();
syncHazeControls();
readiness();
