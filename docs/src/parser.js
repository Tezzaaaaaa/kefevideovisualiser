...
function parseEnhanced(raw) {
  ...
  for (const rawLine of lines) {
    ...
    for (let i = 0; i < words.length; i++) words[i].endTime = i + 1 < words.length ? words[i + 1].time : words[i].time + 0.6;
...
function parseTxt(raw) {
  ...
  return { lines: lines.map(text => ({ time: null, endTime: null, text, words: null })), meta: {} };
}
