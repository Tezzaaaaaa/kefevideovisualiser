...
function projectSnapshot(){return {
  ...
  updated_at:new Date().toISOString(),effect,duration,accent,offset,
...
function loadProject(id){const p=savedProjects().find(x=>x.id===id);if(!p)return;localStorage.setItem(ACTIVE_KEY,p.id);$('#projectName').value=p.name;effect=p.effect||'stack';duration=p.duration||15;accent=p.accent||'#fff';offset=p.offset||0;doc=clone(p.lyrics||{lines:[]});original=clone(p.originalLyrics||p.lyrics||{lines:[]});transformState=clone(p.transform||{x:0,y:0,scale:1,rotation:0});
 document.documentElement.style.setProperty('--accent',accent);$('#colorBtn').style.background=accent;$('#offset').value=offset;$('#offsetLabel').textContent=`${offset>0?'+':''}${offset} ms`;$('#songName').textContent=p.songName||'Original audio';
