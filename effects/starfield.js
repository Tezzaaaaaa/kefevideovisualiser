...
window.kefeEffects.starfield=function(ctx,w,h,style,lines,time){
 ...
 const contract=u.contract('pulse');
 ...
 for(let n=-3;n<visible;n++){
   ...
   const size=u.fitContractText(ctx,'pulse',word.text,base*scale,w*.76);
   ...
   u.drawTrackedText(ctx,word.text,w/2,y,size*(contract.tracking||0),'fillText');
 }
 ctx.restore();
};
})();
