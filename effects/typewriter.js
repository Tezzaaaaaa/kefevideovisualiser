...
window.kefeEffects.typewriter=function(ctx,w,h,style,lines,time){
  ...
  ctx.globalAlpha=.08;ctx.shadowColor='rgba(0,0,0,.65)';ctx.shadowBlur=size*.035;ctx.fillStyle='#000';
  ...
  for(let i=0;i<words.length;i++){
    ...
    const lead=Math.min(.035,Math.max(.012,duration*.04));
    ...
    caretVisible=time>=start-lead&&time<=Math.min(end+.08,a.line.endTime+.08)&&(Math.floor(elapsed*9)%2===0);
    if(caretVisible)caretX=Math.min(cursor+widths[i],cursor+typedWidth);
    cursor+=widths[i]+spacing;
  }
  if(caretVisible){ctx.save();ctx.globalAlpha=.9;ctx.fillStyle=style.textColor||'#F5F2EA';ctx.fillRect(Math.round(caretX+size*.035),y-size*.43,Math.max(1,size*.025),size*.86);ctx.restore();}
  ctx.restore();
};
})();
