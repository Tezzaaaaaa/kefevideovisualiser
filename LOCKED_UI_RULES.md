...
- Mobile order is Preview first, then the tool navigation and active tool inspector, then lyric-detail tools where relevant.
- Mobile/tablet tool navigation stays in normal page flow directly above the active inspector; it must never float over or cover editable content.
...
- `qa/editor-shell-layout-lock.mjs` locks the overall adaptive editor hierarchy, density, single export action, contextual sidebars, mobile order, non-overlapping tool navigation, and overflow behaviour.
