...
async function fetchBlobURL(url, mime, label) {
    ...
    try { const response = await fetch(url, { signal: controller.signal, cache: 'no-store' }); if (!response.ok) throw new Error(`HTTP ${response.status}`); const blob = await response.blob(); if (!blob.size) throw new Error('empty response'); return URL.createObjectURL(new Blob([blob], { type: mime })); }
...
export function releaseEncoder(encoder) { if (encoderPromise) encoderPromise = null; try { const urls = encoder?.__kefeRuntimeURLs; encoder?.terminate?.(); if (urls?.coreURL) URL.revokeObjectURL(urls.coreURL); if (urls?.wasmURL) URL.revokeObjectURL(urls.wasmURL); } catch {} }
