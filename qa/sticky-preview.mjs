...
for(const [name,type] of targets){
  ...
  const css=await page.locator('.preview-sticky-shell').evaluate(el=>({position:getComputedStyle(el).position,top:getComputedStyle(el).top}));
  assert.equal(css.position,'sticky',`${name}: preview is not sticky`);
  assert.equal(css.top,'58px',`${name}: desktop sticky offset changed`);

  await page.locator('#resetProjectVisible').scrollIntoViewIfNeeded();
  await page.waitForTimeout(80);
  const pinned=await page.locator('.preview-sticky-shell').evaluate(el=>({top:el.getBoundingClientRect().top,scrollY:window.scrollY}));
  assert.ok(pinned.scrollY>100,`${name}: page did not scroll far enough to test sticky preview`);
  assert.ok(pinned.top>=56&&pinned.top<=60,`${name}: preview did not remain pinned while editing; top=${pinned.top}`);
  await browser.close();
}
...
await page.goto(base,{waitUntil:'networkidle'});
await page.waitForFunction(()=>document.documentElement.dataset.linaReady==='true',{timeout:20000});
const mobileCss=await page.locator('.preview-sticky-shell').evaluate(el=>({position:getComputedStyle(el).position,top:getComputedStyle(el).top}));
...
await page.locator('#resetProjectVisible').scrollIntoViewIfNeeded();
await page.waitForTimeout(80);
const mobilePinned=await page.locator('.preview-sticky-shell').evaluate(el=>({top:el.getBoundingClientRect().top,scrollY:window.scrollY}));
