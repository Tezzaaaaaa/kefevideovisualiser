...
function wav(seconds=2.4,rate=44100){const count=Math.floor(seconds*rate),bytes=count*2,b=Buffer.alloc(44+bytes);b.write('RIFF');b.writeUInt32LE(36+bytes,4);b.write('WAVE',8);b.write('fmt ',12);b.writeUInt32LE(16,16);b.writeUInt16LE(1,20);b.writeUInt16LE(1,22);b.writeUInt32LE(rate,24);b.writeUInt32LE(rate*2,28);b.writeUInt16LE(2,32);b.writeUInt16LE(16,34);b.write('data',36);b.writeUInt32LE(bytes,40);for(let i=0;i<count;i++)b.writeInt16LE(Math.round(Math.sin(2*Math.PI*440*i/rate)*6000),44+i*2);return b}
const bg=Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1920"><rect width="1080" height="1920" fill="#111111"/></svg>');
...
const brightCount=()=>page.locator('#stageCanvas').evaluate(canvas=>{const data=canvas.getContext('2d',{willReadFrequently:true}).getImageData(0,0,canvas.width,canvas.height).data;let count=0;for(let i=0;i<data.length;i+=32)if(data[i]+data[i+1]+data[i+2]>560)count++;return count});
...
assert.equal(await page.evaluate(()=>document.fonts.check('82px "Homemade Apple"')),true,'Homemade Apple did not load');
...
chooserPromise=page.waitForEvent('filechooser');await page.click('label[for="backgroundInput"]');chooser=await chooserPromise;await chooser.setFiles({name:'dark.svg',mimeType:'image/svg+xml',buffer:bg});
...
assert.ok(early>0,'Eternal handwriting did not begin');assert.ok(mid>early,`Eternal handwriting did not reveal continuously: ${JSON.stringify({early,mid})}`);assert.ok(late>mid,`Eternal handwriting did not continue across the line: ${JSON.stringify({mid,late})}`);

const previewTime=await page.locator('#audioEl').evaluate(el=>el.currentTime);const downloadPromise=page.waitForEvent('download',{timeout:15000});await page.click('#exportBottom');const download=await downloadPromise;const path=await download.path();assert.ok(path,'Eternal export file missing');
const streams=execFileSync('ffprobe',['-v','error','-show_entries','stream=codec_type','-of','csv=p=0',path],{encoding:'utf8'}).trim().split(/\s+/);assert.ok(streams.includes('video')&&streams.includes('audio'),'Eternal export must contain video and audio');
const frameTimes=execFileSync('ffprobe',['-v','error','-select_streams','v:0','-show_entries','frame=best_effort_timestamp_time','-of','csv=p=0',path],{encoding:'utf8'}).trim().split(/\s+/).map(Number).filter(Number.isFinite);const duration=Math.max(...frameTimes),cadence=frameTimes.length/Math.max(duration,.001);assert.ok(duration>2,'Eternal export duration invalid');console.log(`eternal: ${cadence.toFixed(1)}fps delivered by headless browser (60fps requested)`);assert.ok(cadence>20,`Eternal export cadence too low: ${cadence.toFixed(1)}fps`);execFileSync('ffmpeg',['-v','error','-i',path,'-f','null','-']);
await page.waitForFunction(()=>document.querySelector('#exportOverlay').classList.contains('hidden'));assert.ok(Math.abs((await page.locator('#audioEl').evaluate(el=>el.currentTime))-previewTime)<.08,'Eternal export did not restore preview time');assert.deepEqual(errors,[]);await browser.close();console.log('ETERNAL SUNSHINE EFFECT QA: PASS');