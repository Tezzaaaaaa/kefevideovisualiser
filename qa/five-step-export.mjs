...
assert.match(rendererSource,/drawAppleTimedWord/);
...
assert.match(indexSource,/media="print"/);
assert.match(mainSource,/ensureActiveEffectFont/);
assert.match(mainSource,/dataset\.linaReady='true'/);
assert.match(mainSource,/fps\s*:\s*60/);
...
const seek=async value=>page.locator('#seek').evaluate((el,next)=>{el.value=String(next);el.dispatchEvent(new Event('input',{bubbles:true}))},value);
...
await page.click('[data-effect="eternal"]');assert.equal(await page.locator('#hazeEnabled').isChecked(),true);await page.click('[data-align="left"]');await page.locator('#lyricSize').fill('65');
await page.click('[data-effect="apple"]');assert.equal(await page.locator('#lyricSize').inputValue(),'90');assert.match(await page.locator('[data-align="left"]').getAttribute('class'),/active/);assert.equal(await page.locator('#hazeEnabled').isChecked(),true);
await page.click('[data-effect="brat"]');assert.equal(await page.locator('#lyricSize').inputValue(),'110');assert.match(await page.locator('[data-align="right"]').getAttribute('class'),/active/);assert.equal(await page.locator('#hazeEnabled').isChecked(),true);
...
assert.equal(await page.evaluate(()=>document.fonts.check('65px "Homemade Apple"')),true);
...
await seek(.05);await page.waitForTimeout(100);const appleEarly=await sample();
await seek(.22);await page.waitForTimeout(100);const appleMid=await sample();
await seek(.62);await page.waitForTimeout(100);const appleLate=await sample();
...
assert.equal(await page.locator('#hazeEnabled').isChecked(),true);assert.equal((await page.locator('#hazeColor').inputValue()).toLowerCase(),'#8ace00');
await seek(.1);await page.waitForTimeout(100);const bratFirst=await sample();
await seek(.4);await page.waitForTimeout(100);const bratSecond=await sample();
...
await seek(2.22);await page.waitForTimeout(100);const bratFull=await sample();
...
await seek(.05);await page.waitForTimeout(100);const eternalEarly=await sample();
await seek(.26);await page.waitForTimeout(100);const eternalMid=await sample();
await seek(.66);await page.waitForTimeout(100);const eternalLate=await sample();
...
for(const effect of ['apple','brat','eternal']){
  await page.click(`[data-effect="${effect}"]`);await seek(.42);await page.waitForTimeout(80);
  const previewTime=await page.locator('#audioEl').evaluate(el=>el.currentTime);
  const downloadPromise=page.waitForEvent('download',{timeout:15000});
  await page.click('#exportBottom');
  const download=await downloadPromise,path=await download.path();assert.ok(path,`${effect}: no export file`);
  ...
  assert.ok(Math.abs((await page.locator('#audioEl').evaluate(el=>el.currentTime))-previewTime)<.08,`${effect}: preview clock not restored`);
}
assert.deepEqual(errors,[]);
await browser.close();
console.log('LINA EFFECTS INTEGRATION QA: PASS');